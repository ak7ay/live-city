...
function runScriptWithEnv(scriptName: string, fixturePath: string, env: Record<string, string>): unknown {
	const fixture = readFileSync(fixturePath);
	const stdout = execFileSync("python3", [resolve(SCRIPTS, scriptName)], {
		input: fixture,
		stdio: ["pipe", "pipe", "pipe"],
...
describe("Chennai scrapers — Daily Thanthi", () => {
	...
	const items = runScriptWithEnv("dailythanthi.py", resolve(FIXTURES, "dailythanthi-stories.rss"), {
	...
	it("date filter excludes everything when TODAY is far enough ahead", () => {
		const empty = runScriptWithEnv("dailythanthi.py", resolve(FIXTURES, "dailythanthi-stories.rss"), {
