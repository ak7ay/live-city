...
for i, item in enumerate(items_raw, 1):
    ...
    print(f'DESC:  {clean(desc.group(1))[:150] if desc else \"\"}')
    print()
"
```
...
**Caution — extraction output size:** At high story counts, the extraction output overflows tool buffers — the tool saves it to a file automatically, so use the Read tool on the saved path. With DESC[:300] at ~46 stories the output hits ~60KB; with DESC[:150] at 49 stories it was 46KB (2026-04-24, later fetch) — still overflows, but the file-save mechanism handles it. Use `[:150]` on the DESC line to reduce size; the summary field is only used for relevance context, not copied into the output file.
...
- Twitter/social media embed blocks appear mid-article and pass the Kannada-char filter — strip by detecting the attribution line `— AccountName (@handle) Date` (this is the reliable universal signal); also strip the preceding duplicated-content block and any label line before it. Label variants observed: `"ಸಚಿವ [Name] ಟ್ವೀಟ್​"` (minister tweet), `"PIB ಟ್ವಿಟ್​​"` (institutional/govt body tweet, observed 2026-04-25, article 1178098: @PIBBengaluru embed). Image tweets additionally contain `pic.twitter.com` but text-only tweets do not — use the attribution line, not `pic.twitter.com`, as the primary signal (observed 2026-04-11, article 1171538: image tweet; 2026-04-18, article 1174577: text-only tweet from @dineshgrao)
...
- TV9 RSS feed size has grown over time — observed 20 items on 2026-04-10/11, 31 items early on 2026-04-23, 46 items later the same day, and 49 items on 2026-04-24 (new high, later fetch); the window count grows through the day as same-day stories accumulate; treat feed size as unbounded and rely on the date filter, not a slice cap
- TV9 RSS listing routinely includes stories from the previous day — count varies: 6 stories on 2026-04-18 (stories 15–20, ranging 16:43–21:48 IST Apr 17), 25 stories on 2026-04-23 (stories 7–31 early in day, stories 22–46 later — confirmed 25 from Apr 22 all day), 24 from Apr 23 out of 28 total on 2026-04-24 (fetched early), same 24 from Apr 23 out of 49 total even later the same day; count decreases through the day as same-day stories accumulate; filter by date if strict same-day output is needed
...
- Story URLs in the feed are not always under `/karnataka/bengaluru/` — observed `/politics/` (2026-04-18, story 1174934), `/karnataka/mandya/` (story 1174774), `/karnataka/mysuru/` (2026-04-19, story 1175173), `/karnataka/` top-level with no subcategory (2026-04-19, story 1175331), `/education/` top-level (2026-04-21, story 1176105), `/trending/` top-level (2026-04-22, story 1176342), and `/photo-gallery/` top-level (2026-04-26, story 1178554) paths for stories tagged as Bengaluru news; use category tags, not URL path, to identify Bengaluru stories
