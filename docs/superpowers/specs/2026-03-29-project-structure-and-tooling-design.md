...
**This is tooling and infrastructure ONLY** — equivalent to setting up Maven/Gradle with plugins configured, not writing application code.

## Scope (tooling only)

- Corrected `package.json` (deps aligned to DESIGN.md + tooling deps + scripts)
- Biome config (lint + format)
- TypeScript config (updated to match pi-mono base)
- Vitest config (test + coverage)
- Husky pre-commit hook (quality gate)
- Node version pinning (.nvmrc + engines)
- Editor config (.editorconfig)
- Git setup (.gitignore)
- Env template (.env.example)
- Empty directory scaffolding with .gitkeep
- Minimal `src/index.ts` placeholder (so `build` and `check` work)

## Explicitly Out of Scope

- **No application code** — no env.ts, logger.ts, types.ts, errors.ts, extractor.ts, etc.
- Application source files are created later during feature implementation via TDD
...
Only config files and empty scaffolding — no application source files.

```
live-city/
├── src/
│   └── index.ts                 # Minimal placeholder (so build/check work)
├── test/
│   ├── unit/
│   │   └── .gitkeep
│   └── integration/
...
├── config/
│   └── cities/
│       └── .gitkeep             # City YAML configs added during feature work
├── .husky/
...
Application directories (`src/config/`, `src/core/`, `src/extraction/`, `src/scheduler/`, `src/storage/`, `src/utils/`) are created during feature implementation, not upfront.
